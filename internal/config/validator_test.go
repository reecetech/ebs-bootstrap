...
func TestResizeThresholdValidator(t *testing.T) {
	subtests := []struct {
		Name          string
		Config        *Config
		ExpectedError error
		...
		{
			Name: "Invalid Resize Threshold (Defaults)",
			Config: &Config{
				Defaults: Options{
					ResizeThreshold: -5.0,
				},
			...
			ExpectedError: fmt.Errorf("🔴 '-5' (default) must be a floating point between 0 and 100 (inclusive)"),
		},
		{
			Name: "Invalid Resize Threshold (Overrides)",
			Config: &Config{
				Devices: map[string]Device{
					"/dev/xvdf": {},
				},
				overrides: Options{
					ResizeThreshold: 110.0,
				},
			},
			ExpectedError: fmt.Errorf("🔴 '110' (-resize-threshold) must be a floating point between 0 and 100 (inclusive)"),
		},
		{
			Name: "Invalid Resize Threshold (Device)",
			Config: &Config{
				Devices: map[string]Device{
					"/dev/xvdf": {
						Options: Options{
			...
			ExpectedError: fmt.Errorf("🔴 /dev/xvdf: '-1' must be a floating point between 0 and 100 (inclusive)"),
		},
	}
	for _, subtest := range subtests {
		t.Run(subtest.Name, func(t *testing.T) {
			...
			utils.CheckError("rtv.Validate()", t, subtest.ExpectedError, err)
		})
	}
}
